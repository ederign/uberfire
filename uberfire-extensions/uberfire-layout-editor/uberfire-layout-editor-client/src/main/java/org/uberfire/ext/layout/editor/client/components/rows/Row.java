...
public class Row {
    ...
    private void updateHeightOfSiblinColumn(Column columnToRemove) {
        final int removeIndex = getColumnIndex(columnToRemove);
        GWT.log(columnToRemove.getColumnHeight() + " <- to remove height");
        if (isFirstColumn(removeIndex)) {
            final Column sibling = columns.get(1);
            //TOdo maybe put height in integer
            ...
            GWT.log("remove " + remove.toString() + " add " + add.toString() + " n " + newHeight.toString());
        } else {
